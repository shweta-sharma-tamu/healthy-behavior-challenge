...
RSpec.describe PasswordResetsController, type: :controller do
  ...
  it 'resets the password' do
    patch :update, params: { token:, user: { password: 'new_password', password_confirmation: 'new_password' } }
    expect(response).to redirect_to(root_path)
...
RSpec.describe PasswordResetsController, type: :controller do
  ...
  it 'throws error for password is not matching with password confirmation' do
    patch :update, params: { token:, user: { password: 'new_password', password_confirmation: 'new_password1' } }
