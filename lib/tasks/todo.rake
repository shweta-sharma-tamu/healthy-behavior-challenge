...
namespace :mail do
  ...
  task send_reminder_todos_mail: :environment do
    ...
    trainee_ids.each do |id|
      ...
      challenge_ids = TodolistTask.where('trainee_id = ? AND date = ? AND status != ?', id, Date.today,
                                         'completed').distinct.pluck(:challenge_id)
      @chmap = {}
      challenge_ids.each do |challenge_id|
        todos = TodolistTask.where('trainee_id = ? AND date = ? AND status != ? AND challenge_id=?', id, Date.today,
