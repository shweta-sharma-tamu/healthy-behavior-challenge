...
Given('there is a instructor registered with the email {string} and password {string}') do |email, password|
  puts
  @user = User.create!(email:, password:, user_type: 'Instructor')
...
And('Instructor Referral: I click the {string} button') do |_button_text|
...
Given('I receive a referral link from {string}') do |email|
  ...
  user.instructor_referrals.create(email: 'example@example.com', token: @token, is_used: false,
